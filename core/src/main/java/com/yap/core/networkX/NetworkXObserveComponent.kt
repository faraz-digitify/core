...
class NetworkXObserveComponent(
    ...
    private fun checkInternetAndInvokeListener() {
        currentJob = coroutineScope.launch {
            val hasActiveConnection = applicationContext.isConnectedToInternet()
                    && hasActiveInternetConnection()

            if (!hasActiveConnection) {
                launch(Dispatchers.Main) {
                    listener.onDisconnected(applicationContext.isAirplaneModeOn())
                }
            }
        }
    }
